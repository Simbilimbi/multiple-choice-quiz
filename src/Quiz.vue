...
<template>
  <div v-if="quizState != 'completed'">
        <h2 class="p-4 text-4xl border-b border-white">{{ questions[currentIndex].question }}</h2>
        <div v-for="option in questions[currentIndex].options"   class="flex px-12 py-4 text-3xl gap-x-4">
          <label>{{ option }}</label>
